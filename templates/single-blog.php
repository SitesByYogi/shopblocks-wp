...
while ( have_posts() ) : the_post();
	...
	$has_sidebar = ( ( '1' === $newsletter && trim( (string) $newsletter_embed ) ) || ( shopblocks_has_woocommerce() && trim( (string) $product_ids ) ) || trim( (string) $helpful ) || trim( (string) $custom_content ) || trim( (string) shopblocks_render_sidebar_cta( $post_id ) ) || trim( (string) $sidebar_form_shortcode ) );
	?>
		...
		<header class="shopblocks-blog__hero<?php echo $hero_url ? ' has-background' : ' has-no-background'; ?>"<?php if ( $hero_url ) : ?> style="--shopblocks-blog-hero-image:url('<?php echo esc_url( $hero_url ); ?>')"<?php endif; ?>>
			...
			<div class="shopblocks-blog__hero-shell">
				<div class="shopblocks-blog__hero-content">
					...
					<div class="shopblocks-blog__meta"><time class="shopblocks-blog__date" datetime="<?php echo esc_attr( get_the_date( DATE_W3C ) ); ?>"><?php echo esc_html( get_the_date() ); ?></time><span class="shopblocks-blog__meta-separator" aria-hidden="true">|</span><span class="shopblocks-blog__author"><?php echo esc_html( get_the_author() ); ?></span></div>
					<?php if ( ( $primary_label && $primary_url ) || ( $secondary_label && $secondary_url ) ) : ?><div class="shopblocks-hero-actions"><?php if ( $primary_label && $primary_url ) : ?><a class="shopblocks-button shopblocks-button--primary" href="<?php echo esc_url( $primary_url ); ?>"><?php echo esc_html( $primary_label ); ?></a><?php endif; ?><?php if ( $secondary_label && $secondary_url ) : ?><a class="shopblocks-button shopblocks-button--secondary" href="<?php echo esc_url( $secondary_url ); ?>"><?php echo esc_html( $secondary_label ); ?></a><?php endif; ?></div><?php endif; ?>
				</div>
				<?php if ( $hero_form_shortcode ) : ?><div class="shopblocks-blog__hero-conversion"><?php echo shopblocks_render_lead_form( $post_id, '_shopblocks_hero_form_shortcode' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div><?php endif; ?>
			</div>
		</header>
		<div class="shopblocks-blog__layout">
			<div class="shopblocks-blog__main"><article class="shopblocks-blog__article"><div class="shopblocks-gutenberg-content shopblocks-blog__content"><?php the_content(); ?></div></article><?php echo shopblocks_render_faqs( $faqs, $faq_heading ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div>
			<?php if ( $has_sidebar ) : ?>
			<aside class="shopblocks-blog__sidebar" aria-label="<?php esc_attr_e( 'Blog sidebar', 'shopblocks-wp' ); ?>">
				...
				<?php if ( '1' === $newsletter && trim( (string) $newsletter_embed ) ) : ?><section class="shopblocks-sidebar-block shopblocks-sidebar-block--newsletter"><?php if ( $newsletter_title ) : ?><h2 class="shopblocks-newsletter__title"><?php echo esc_html( $newsletter_title ); ?></h2><?php endif; ?><?php if ( $newsletter_text ) : ?><p class="shopblocks-newsletter__description"><?php echo esc_html( $newsletter_text ); ?></p><?php endif; ?><div class="shopblocks-newsletter__form"><?php echo shopblocks_render_embed_content( $newsletter_embed ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></div></section><?php endif; ?>
